...
class Convolution:
    ...
    def _convolve_normalised(self) -> None:
        ...
        # APPROXIMATION 3D uniform kernel (convolution == correlation)
    ...
    def _convolve(self, data: np.ndarray, borders: str, output: np.ndarray | None = None) -> None:
        ...
        if data.ndim == 2:
            kernel = np.flip(self._kernel).astype(data.dtype)
            cv2.filter2D(
                data,
            ...
            # Convolution and correlation are the same in this case.
            kernel_2d = np.ones(
                (self._kernel.shape[1], self._kernel.shape[2]),
                dtype=data.dtype,
            ) * self._kernel.flat[0]
